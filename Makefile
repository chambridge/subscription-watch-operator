...
help:
	...
	@echo "  deploy-custom-resources             deploy the subscription watch custom resources to trigger the operator roles"
...
build-operator-image:
	operator-sdk build quay.io/$(username)/subs-watch-operator:v0.0.1
	docker push quay.io/$(username)/subs-watch-operator:v0.0.1
...
deploy-operator-quay-user:
	...
	sed -i "" 's|{{ REPLACE_IMAGE }}|quay.io/$(username)/subs-watch-operator:v0.0.1|g' testing/operator.yaml
...
deploy-dependencies:
	oc apply -f deploy/crds/subs_watch_crd.yaml || true
	oc apply -f deploy/crds/subs_watch_data_crd.yaml || true
...
deploy-custom-resources:
	oc apply -f testing/subs_watch_cr.yaml
	oc apply -f deploy/crds/subs_watch_data_cr.yaml

delete-dependencies-and-resources:
	oc delete -f testing/subs_watch_cr.yaml
	oc delete -f deploy/crds/subs_watch_data_cr.yaml
	oc delete -f deploy/crds/subs_watch_crd.yaml
	oc delete -f deploy/crds/subs_watch_data_crd.yaml
...
delete-metering-report-resources:
	...
	oc delete report --selector subscription-watch=true || true
...
setup-operator:
	@cp deploy/crds/subs_watch_cr.yaml testing/subs_watch_cr.yaml
	@sed -i "" 's/cluster-id-placeholder/$(clusterID)/g' testing/subs_watch_cr.yaml
	@sed -i "" 's/reporting-operator-token-placeholder/$(report_token_name)/g' testing/subs_watch_cr.yaml
	@sed -i "" 's/false/$(shell echo $(or $(validate_cert),false))/g' testing/subs_watch_cr.yaml
	@sed -i "" 's/token/$(shell echo $(or $(authentication),token))/g' testing/subs_watch_cr.yaml
	@sed -i "" 's|https://cloud.redhat.com/api/ingress/v1/upload|$(shell echo $(or $(ingress_url),https://cloud.redhat.com/api/ingress/v1/upload))|g' testing/subs_watch_cr.yaml
