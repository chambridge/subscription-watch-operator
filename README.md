...
If you have pushed your changes to a branch within the repository then an associated image branch will have been built in [quay.io/chambridge/subscription-watch-operator](https://quay.io/chambridge/subscription-watch-operator). You need only specify the branch you want to deploy the operator with using the following command:
...
2. After all changes to the operator have been merged into master, cut a release with the new version tag. This will kick start an quay image build with the new release version [here](https://quay.io/repository/chambridge/subs-watch-operator?tab=tags). Use this image to replace the previous image in the ClusterServiceVersion.
